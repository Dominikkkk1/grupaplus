 ...
 */
export const POST = withAuth(["admin", "operator"], async (request, { supabase, user, role }) => {
  const parsed = await parseBody(request);
  if ("error" in parsed) return parsed.error;

  const body = parsed.data as Record<string, unknown>;
  const { progressId, action, machineId } = body as { progressId: string; action: string; machineId?: string };
  const force = body.force && role === "admin";
  console.log("[SCAN] action=%s progressId=%s force=%s machineId=%s user=%s role=%s", action, progressId, force, machineId, user.id, role);
