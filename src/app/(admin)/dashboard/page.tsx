...
export default async function DashboardPage() {
  ...
  const [
    ...
    // Operatorzy
    supabase.from("order_item_progress").select("completed_by, started_at, completed_at, status").in("status", ["completed", "in_progress"]).gte("completed_at", thirtyDaysAgo.toISOString()).not("completed_by", "is", null),
    supabase.from("users").select("id, full_name").in("role", ["admin", "operator"]).eq("is_active", true),
    // Reklamacje
    supabase.from("complaints").select("id", { count: "exact", head: true }).eq("status", "open"),
    supabase.from("complaints").select("id", { count: "exact", head: true }).in("status", ["resolved", "rejected"]).gte("resolved_at", thirtyDaysAgo.toISOString()),
    supabase.from("orders").select("id", { count: "exact", head: true }).gte("created_at", thirtyDaysAgo.toISOString()),
    // Top maszyny
    supabase.from("order_item_progress").select("machine_id, machine:machines(name)").eq("status", "completed").gte("completed_at", thirtyDaysAgo.toISOString()).not("machine_id", "is", null),
  ]);
  ...
  const operatorMap = new Map<string, { completed: number; totalSeconds: number; inProgress: number }>();
  for (const row of operatorData ?? []) {
    ...
    if (row.status === "completed") {
      ...
      if (row.started_at && row.completed_at) {
        const seconds = (new Date(row.completed_at).getTime() - new Date(row.started_at).getTime()) / 1000;
        if (seconds > 0 && seconds < 86400) stats.totalSeconds += seconds;
      }
    }
    if (row.status === "in_progress") stats.inProgress++;
  }
  ...
  const operators = Array.from(operatorMap.entries())
    .map(([userId, stats]) => ({
      ...
      avgMinutes: stats.completed > 0 ? Math.round(stats.totalSeconds / stats.completed / 60) : 0,
  ...
  for (const row of machineData ?? []) {
    ...
    if (!machineCounts.has(mid)) machineCounts.set(mid, { name: mname, count: 0 });
    machineCounts.get(mid)!.count++;
  }
  const topMachines = Array.from(machineCounts.values()).sort((a, b) => b.count - a.count).slice(0, 5);
