      ...
      {error ? (
                ...
                return (
                  <tr key={order.id} className="border-b border-zinc-100 last:border-0 transition-colors hover:bg-zinc-50/50">
                    <td className="px-4 py-3">
                      <a href={`/orders/${order.id}`} className="font-mono text-[13px] font-medium text-zinc-900 hover:text-blue-600">
                        {order.order_number}
                      </a>
                    </td>
                    ...
                    <td className="px-4 py-3 text-[13px] text-zinc-600">
                      {order.payment_status === "paid" ? "Oplacone" : order.payment_status === "cod" ? "Za pobraniem" : "Oczekuje"}
                    </td>
        ...
        <div className="rounded-lg border border-zinc-200 bg-white p-16 text-center shadow-sm">
          ...
          <p className="mt-1 text-[13px] text-zinc-500">
            Kliknij &quot;Nowe zamowienie&quot; aby dodac reczne zlecenie lub podlacz WooCommerce.
          </p>
        </div>
      )}
    </div>
